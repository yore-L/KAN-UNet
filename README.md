# KAN-UNet: Semantic segmentation with PyTorch
...
## Description
This model introduces innovative improvements based on the traditional UNet architecture by incorporating Kolmogorov-Arnold Network (KAN) modules into the encoder downsampling path and the bottleneck layer. The KAN enhances the model's ability to fit complex features through adaptive activation functions. The decoder still consists of transposed convolutional layers (or upsampling layers), aiming to restore the spatial resolution of the image. After each upsampling step, skip connections are used to combine the corresponding feature maps from the encoder with the current decoder feature maps, helping to preserve image details.
...
```
python predict.py -m ./checkpoints/model.pth -i /data/Test -o output_masks
```
...
`python predict.py ./checkpoints/model.pth -i /data/Test -o output_masks --viz --no-save`
